#!/bin/sh
kubectl delete namespace student31-bookinfo-dev
kubectl delete namespace student31-bookinfo-uat
kubectl delete namespace student31-bookinfo-prd
gcloud container clusters get-credentials k8s --project zcloud-cicd --zone asia-southeast1-a
kubectl create namespace student31-bookinfo-dev
kubectl create namespace student31-bookinfo-uat
kubectl create namespace student31-bookinfo-prd
kubectl config set-context $(kubectl config current-context) --namespace=student31-bookinfo-uat
...
kubectl config set-context $(kubectl config current-context) --namespace=student31-bookinfo-prd
kubectl create secret generic registry-bookinfo --from-file=.dockerconfigjson=$HOME/.docker/config.json --type=kubernetes.io/dockerconfigjson
...
kubectl config set-context $(kubectl config current-context) --namespace=student31-bookinfo-dev
kubectl create secret generic registry-bookinfo --from-file=.dockerconfigjson=$HOME/.docker/config.json --type=kubernetes.io/dockerconfigjson
